...
*/
$MONGO["servers"][$i]["mongo_name"] = getenv("MONGO_SERVER") ?? "Localhost";//mongo server name
//$MONGO["servers"][$i]["mongo_sock"] = "/var/run/mongo.sock";//mongo socket path (instead of host and port)
$MONGO["servers"][$i]["mongo_host"] = getenv("MONGO_SERVER") ?? "127.0.0.1";//mongo host
$MONGO["servers"][$i]["mongo_port"] = getenv("MONGO_PORT") ?? "27017";//mongo port
$MONGO["servers"][$i]["mongo_timeout"] = 0;//mongo connection timeout
$MONGO["servers"][$i]["mongo_db"] = getenv("MONGO_DATABASE");//default mongo db to connect, works only if mongo_auth=false
$MONGO["servers"][$i]["mongo_user"] = getenv("MONGO_USERNAME");//mongo authentication user name, works only if mongo_auth=false
$MONGO["servers"][$i]["mongo_pass"] = getenv("MONGO_PASSWORD");//mongo authentication password, works only if mongo_auth=false
$MONGO["servers"][$i]["mongo_auth"] = getenv("MONGO_AUTH") ?? false;//enable mongo authentication?

$MONGO["servers"][$i]["control_auth"] = getenv("ROCKMONGO_AUTH");//enable control users, works only if mongo_auth=false
